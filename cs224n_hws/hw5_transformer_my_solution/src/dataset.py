...
class CharCorruptionDataset(Dataset):
    def __init__(self, data, block_size):
        ...
        print('unfiltered dataset size=' + str(len(self.data)))
        self.data = [s for s in self.data if len(s) >= 4]
        print('filtered dataset size=' + str(len(self.data)))
    ...
    def __getitem__(self, idx):
        ...
        # retrieve 'document' as the 'idx' line of 'data'
        document = self.data[idx]
        # truncate 'document' to random length
        max_length = min(int(self.block_size * 7 / 8), len(document))
        random_length = random.randint(4, max_length)
        document = document[:random_length]
        # separate 'document' into 'prefix + masked_content + suffix'
        prefix_length = int(len(document) * 1 / 4)
        random_length = random.randint(max(1, int(len(document) * 1 / 8)), 
            int(len(document) * 3 / 8))
        if prefix_length + random_length > len(document):
            random_length = len(document) - prefix_length - 1
        prefix = document[:prefix_length]
        masked_content = document[prefix_length:prefix_length + random_length]
        suffix = document[prefix_length + random_length:]
        # put masked_content at the end to form masked_string
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR +                 masked_content
        masked_string = masked_string + self.PAD_CHAR * (self.block_size - len(masked_string))
        # cut off last/first character to form input/output examples
        x = masked_string[:-1]
        y = masked_string[1:]
        # encode input and output as long tensors
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
        return x, y
