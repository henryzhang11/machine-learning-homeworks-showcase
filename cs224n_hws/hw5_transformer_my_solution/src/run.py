...
if args.variant == 'vanilla':
    ...
    model = model.GPT(mconf)  # Placeholder for your model creation code
...
if args.function == 'pretrain':
    ...
    #     num_workers=4
    # Placeholder for pretraining code
    config = trainer.TrainerConfig(
        ...
        num_workers = 2 # google won't let me create 4 workers
    )
    pretrainer = trainer.Trainer(model, pretrain_dataset, None, config)
    pretrainer.train()
    ...
    train_dataset = dataset.NameDataset(pretrain_dataset, open(
        args.finetune_corpus_path).read())
    finetuner = trainer.Trainer(model, train_dataset, None, config)
    finetuner.train()
    ...
    model.load_state_dict(torch.load(args.reading_params_path, map_location
        =torch.device('cpu')))
