...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        """
        Y_i = softmax(ReLU(XA_i)B_i+b_2)(XV_i)
        X.shape == (B, T, C)
        A_i.shape == (C, hs)
        w1.shape == (C, C)
        XA_i.shape == (B, i, T, hs)
        score.shape == (B, T, C)
        reshaped_score.shape == (B, nh, T, hs)
        (self.w2).shape == (hs, T)
        (self.b2).shape == (T,)
        (F.relu(reshape_score) @ self.w2+self.b2[None, None, None, :]).shape == (B, nh, T, T)
        """
        score = self.w1(x) # (B, T, C) x (C, C) -> (B, T, C)
        reshaped_score = score.view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        att = F.relu(reshaped_score) @ self.w2[:, :T] + self.b2[None, None, None, :T] # (B, nh, T, T)
